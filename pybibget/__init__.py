...
def pybibget() -> None:
    ...
    parser.add_argument('keys', type = str, metavar = 'citekeys', nargs='*',
    help ='MathSciNet (MRxxxxx), arXiv (xxxx.xxxxx), PubMed (PMID:xxxxxxxx)'\
    'or DOI (10.xxx/xxxxx) citation keys (separated by spaces)')
    parser.add_argument('-v','--verbose',action='store_true',help='verbose output')
    parser.add_argument('-f', action='store',dest='file_name',
    help='Append output to file (default: write output to stdout)')
...
def pybibparse():
    ...
    """
    parser = argparse.ArgumentParser(prog ='pybibget',
    description ='Command line utility to automatically retrieve missing BibTeX citations'\
        'from MathSciNet, arXiv and PubMed')

    parser.add_argument('file_name', type = str, metavar ='file name', nargs=1, 
    help ='base file name of main tex file (without .tex extension)')
    parser.add_argument('-v','--verbose',action='store_true',help='verbose output')
    parser.add_argument('-w','--write', metavar="file_name",action='store', nargs='?', const=" ", 
    type=str, help='Append output to .bib file (default: write output to stdout). The .bib '\
        'file is automatically detected from the .blg file. If no .bib file is found, '\
        'the .bib file has to be specified explicitly via "-w file_name.bib".')
    ...
    with open(base_file_name+".blg") as f:
        blg_file = f.read()
    missing_cites = re.findall(r"I didn't find a database entry for '([A-Za-z0-9\.\-_ :\/]*)'", blg_file) + re.findall(r'I didn\'t find a database entry for "([A-Za-z0-9\.\-_ :\/]*)"', blg_file)
