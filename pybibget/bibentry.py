...
async def getbibentry(key, verbose=False):
    """
    ...
    if re.match(RE_MSC, key):
        ...
        try:
            result = await get_mathscinet_bibentry(mrkey=key, verbose=verbose)
        except ValueError as err:
            print(err)
    elif re.match(RE_PMID, key):
        if verbose:
            print(f"Looking for {key} on PubMed...", end=" ")
        result = await get_pubmed_bibentry(pmid=key, verbose=verbose)
    elif re.match(RE_ARXIV_OLD, key) or re.match(RE_ARXIV_NEW, key):
        if verbose:
            print(f"Looking for arXiv:{key}...", end=" ")
        result = await get_arxiv_bibentry(key, verbose=verbose)
    elif re.match(RE_DOI, key):
        try:
            if verbose:
                print(f"Looking for {key} on MathSciNet...", end=" ")
            result = await get_mathscinet_bibentry(doi=key, verbose=verbose)
        except ValueError:
            if verbose:
                print(f"Not found. Looking for {key} on doi.org...", end=" ")
            result = await get_doi_bibentry(key, verbose=verbose)
    else:
        raise ValueError(f"Invalid citation key {key}!\n")
    return (result, key)


async def get_mathscinet_bibentry(mrkey=None, doi=None, verbose=False):
    """
...
async def get_doi_bibentry(doi, verbose=False):
    """
...
async def get_arxiv_bibentry(arxiv_key, verbose=False):
    """
    ...
    async with httpx.AsyncClient() as client:
        ...
        if doi := tree.xpath("//a:entry/b:doi", namespaces={'a': ATOM, 'b': ARXIV}):
            ...
            bibentry, _ = await getbibentry(doi[0].text, verbose=verbose)
...
async def get_pubmed_bibentry(pmid, verbose=False):
    """
    ...
    async with httpx.AsyncClient() as client:
        ...
        if page.status_code == 200:
            ...
            try:
                ...
                doi_entry, _ = await getbibentry(doi, verbose=verbose)
